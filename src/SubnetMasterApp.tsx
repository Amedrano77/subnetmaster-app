...
import {
  ChevronRight, Lock, Building, Wifi, Users, Globe, Briefcase,
  Trophy, Star, Play, Home, Target, Clock, CheckCircle, Check, X
} from 'lucide-react';

const SubnetMasterApp = () => {
  const [currentView, setCurrentView] = useState<'intro' | 'building' | 'level'>('intro');
  ...
  const [gameProgress, setGameProgress] = useState(() => {
    try {
      ...
      return saved
        ? JSON.parse(saved)
        : { unlockedLevels: [1], completedLevels: [], achievements: [], totalScore: 0 };
    } catch {
      return { unlockedLevels: [1], completedLevels: [], achievements: [], totalScore: 0 };
    }
  });
  ...
  const generateLevel1Scenario = () => {
    const scenarios = [
      { baseNetwork: '192.168.1.0/24',  requiredSubnets: 4, description: 'Split into 4 equal subnets', story: 'The reception area needs 4 separate network segments for different departments.' },
      { baseNetwork: '192.168.10.0/24', requiredSubnets: 2, description: 'Split into 2 equal subnets', story: 'Create separate networks for employees and guests.' },
      { baseNetwork: '192.168.50.0/24', requiredSubnets: 8, description: 'Split into 8 equal subnets', story: 'Each workgroup needs its own network segment.' }
    ];
    return scenarios[Math.floor(Math.random() * scenarios.length)];
  };

  const calculateLevel1Solution = (baseNetwork: string, requiredSubnets: number) => {
  ...
  const validateLevel1Answer = (userAnswer: string, correctSolution: any, field: string) => {
  ...
  const startLevel = (level: any) => {
    ...
    setFeedback([]);
    setScore(0);
    if (level.id === 1) setScenario(generateLevel1Scenario());
  };
  ...
  const submitSubnetAnswer = () => {
    ...
    const correctCount = Object.values(validations).filter((v: any) => v.isCorrect).length;
    const subnetScore = Math.round((correctCount / 5) * 100);
    setFeedback(Object.values(validations).map((v: any) => v.feedback));

    if (currentSubnet < scenario.requiredSubnets - 1) {
      setTimeout(() => {
        setCurrentSubnet((s) => s + 1);
        setFeedback([]);
      }, 3000);
    } else {
  ...
  const updateSubnetAnswer = (field: string, value: string) => {
    setUserAnswers((prev) => ({
      ...prev,
      [currentSubnet]: { ...(prev[currentSubnet] || {}), [field]: value },
    }));
  };

  const levels = [
    { id: 1, title: 'Small Office Network', subtitle: 'Class C Basics', description: 'Help the ByteTech small office get their basic network running', icon: <Home className="w-8 h-8" />, color: 'bg-gradient-to-br from-blue-500 to-blue-600', difficulty: 'Beginner', estimatedTime: '10 minutes', floor: '1st Floor - Reception' },
    { id: 2, title: 'Department Networks', subtitle: 'Class C VLSM', description: 'Design efficient networks for different departments', icon: <Users className="w-8 h-8" />, color: 'bg-gradient-to-br from-green-500 to-green-600', difficulty: 'Intermediate', estimatedTime: '15 minutes', floor: '2nd Floor - Departments' },
    { id: 3, title: 'Corporate Campus', subtitle: 'Class B Networks', description: 'Scale up to enterprise-level networking', icon: <Building className="w-8 h-8" />, color: 'bg-gradient-to-br from-purple-500 to-purple-600', difficulty: 'Advanced', estimatedTime: '20 minutes', floor: '3rd Floor - Corporate' },
    { id: 4, title: 'Enterprise Solutions', subtitle: 'Class B VLSM', description: 'Master complex enterprise network design', icon: <Wifi className="w-8 h-8" />, color: 'bg-gradient-to-br from-orange-500 to-orange-600', difficulty: 'Expert', estimatedTime: '25 minutes', floor: '4th Floor - Enterprise' },
    { id: 5, title: 'Global Operations', subtitle: 'Class A Mastery', description: 'Architect networks for millions of users worldwide', icon: <Globe className="w-8 h-8" />, color: 'bg-gradient-to-br from-red-500 to-red-600', difficulty: 'Master', estimatedTime: '30 minutes', floor: '5th Floor - Global HQ' },
    { id: 6, title: 'Consulting Practice', subtitle: 'Multi-Client Projects', description: 'Manage multiple client networks simultaneously', icon: <Briefcase className="w-8 h-8" />, color: 'bg-gradient-to-br from-indigo-500 to-indigo-600', difficulty: 'Expert Consultant', estimatedTime: '35 minutes', floor: '6th Floor - Executive Suite' },
  ];
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 text-white">
      ...
      <div className="container mx-auto px-4 py-8 relative z-10">
            ...
            <div className="grid lg:grid-cols-2 gap-10 items-center">
              <div>
                ...
                <div className="space-y-6 text-lg leading-relaxed">
                  <p className="text-gray-200">You&apos;re <span className="font-bold text-blue-300">Alex</span>, a new IT apprentice at ByteTech Industries.</p>
                  <p className="text-gray-200">Your mentor <span className="text-green-400 font-bold">NetBot 🤖</span> will guide you through increasingly complex subnetting puzzles.</p>
                  <p className="text-gray-200">Each floor presents unique networking challenges. Complete each level to unlock your keycard and advance!</p>
                </div>
              </div>
                ...
                <div className="bg-gradient-to-br from-blue-500/20 to-purple-500/20 rounded-2xl p-8 border border-blue-400/30 shadow-xl">
                  <div className="text-center mb-6">
                    ...
                    <div className="mt-2 bg-blue-500/20 rounded-full h-2">
                      <div className="bg-blue-400 h-2 rounded-full w-1/4 animate-pulse" />
                    </div>
                  </div>
        ...
        <div className="grid lg:grid-cols-3 gap-8 mb-16">
          <Feature icon={<Trophy className="w-8 h-8 text-white" />} title="Progressive Mastery" text="From Class C basics to global Class A designs across 6 levels." />
          <Feature icon={<Target className="w-8 h-8 text-white" />} title="Real-World Scenarios" text="Solve practical problems with instant feedback and guidance." />
          <Feature icon={<Star className="w-8 h-8 text-white" />} title="Gamified Experience" text="Unlock floors, earn achievements, and track your progress." />
        </div>
        ...
        <div className="flex flex-col lg:flex-row lg:justify-between lg:items-center mb-12">
          ...
          <div className="bg-white/10 backdrop-blur-md rounded-2xl p-6 border border-white/20 shadow-xl">
            <div className="flex items-center space-x-8">
              <Stat label="Floors Cleared" value={gameProgress.completedLevels.length} color="text-green-400" />
              <Stat label="Total Score" value={gameProgress.totalScore} color="text-blue-400" />
              <Stat label="Achievements" value={0} color="text-purple-400" />
            </div>
          </div>
        </div>
            ...
            {levels.map((level, index) => {
              ...
              return (
                <div key={level.id} className="relative group">
                  <div
                    className={`bg-white/10 backdrop-blur-md rounded-2xl p-8 border transition-all duration-500 shadow-xl ${
                      isUnlocked
                  ...
                  >
                    <div className="flex items-center justify-between">
                      <div className="flex items-center space-x-6">
                        <div className={`p-4 rounded-2xl text-white flex items-center justify-center shadow-lg ${level.color} transition-transform duration-300 ${isUnlocked ? 'group-hover:scale-110' : ''}`}>
                          {level.icon}
                        </div>
                        <div className="flex-1">
                          <div className="flex items-center space-x-4 mb-3">
                            <h3 className="text-2xl font-bold">{level.title}</h3>
                            <span className="text-sm px-3 py-1 rounded-full font-medium bg-blue-500/20 text-blue-300">{level.subtitle}</span>
                            {isCompleted && (
                          ...
                          <div className="flex items-center space-x-6 text-sm text-gray-400">
                            <div className="flex items-center"><span className="mr-2">📍</span><span>{level.floor}</span></div>
                            <div className="flex items-center"><Clock className="w-4 h-4 mr-2" /><span>{level.estimatedTime}</span></div>
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="flex items-center space-x-4">
                        {isUnlocked ? (
                          ...
                          >
                            {isCompleted ? (<><Trophy className="mr-2 w-5 h-5" /> Replay Level</>) : (<><Play className="mr-2 w-5 h-5" /> Start Challenge</>)}
                  ...
                  {index < levels.length - 1 && (
                    <div className="flex justify-center py-3">
                      <div className={`w-1 h-6 rounded-full transition-all duration-500 ${gameProgress.completedLevels.includes(level.id) ? 'bg-gradient-to-b from-green-400 to-blue-500 shadow-lg' : 'bg-gray-600'}`} />
                    </div>
                  )}
                </div>
              );
  ...
  const LevelScreen = () => {
            ...
            <div className="max-w-4xl mx-auto">
              <div className="bg-white/10 backdrop-blur-md rounded-xl p-8 border border-white/20">
                <div className="text-center mb-8">
                  <div className={`inline-block p-4 rounded-xl text-white mb-4 ${currentLevel.color}`}>{currentLevel.icon}</div>
                  ...
                  <div className="flex items-start space-x-4">
                    ...
                    <div>
                      ...
                      <div className="text-green-100 space-y-2">
                        ...
                        <p>"You’ll calculate the new subnet mask and provide details for each subnet."</p>
    ...
    if (gameState === 'playing') {
      const solution = useMemo(() => (
        calculateLevel1Solution(scenario.baseNetwork, scenario.requiredSubnets)
        ...
        <div className="min-h-screen bg-gradient-to-br from-gray-900 via-blue-900 to-purple-900 text-white">
          <div className="container mx-auto px-4 py-8">
            ...
            <div className="max-w-4xl mx-auto">
              ...
              {/* SINGLE form card with tab order and one submit button */}
              <div className="bg-white/10 backdrop-blur-md rounded-xl p-8 border border-white/20">
                <h3 className="text-2xl font-bold mb-6 text-center">Configure Subnet {currentSubnet + 1}</h3>
                ...
                <div className="grid md:grid-cols-2 gap-6">
                  <Input
                    ...
                    onBlur={(v) => updateSubnetAnswer('networkAddress', v)}
                  />
                  <Input
                    label="Subnet Mask"
                    ...
                    onBlur={(v) => updateSubnetAnswer('subnetMask', v)}
                  />
                  <Input
                    ...
                    onBlur={(v) => updateSubnetAnswer('broadcastAddress', v)}
                  />
                  <Input
                    label="First Host"
                    ...
                    onBlur={(v) => updateSubnetAnswer('firstHost', v)}
                  />
                  <Input
                    label="Last Host"
                    ...
                    onBlur={(v) => updateSubnetAnswer('lastHost', v)}
                  />
                </div>
          ...
          <div className="container mx-auto px-4 py-8">
            <div className="max-w-4xl mx-auto text-center">
              <div className="bg-white/10 backdrop-blur-md rounded-xl p-8 border border-white/20">
                ...
                <h1 className="text-4xl font-bold mb-4">{passed ? 'Congratulations!' : 'Keep Practicing!'}</h1>
                <div className="text-6xl font-black mb-4"><span className={passed ? 'text-green-400' : 'text-orange-400'}>{score}%</span></div>
                <p className="text-xl mb-8">
                  {passed ? `Amazing work! You’ve mastered ${currentLevel.title} and earned ${score} points!`
                ...
                {passed && (
                  <div className="bg-green-500/10 border border-green-400/30 rounded-lg p-4 mb-6">
                    <p className="text-green-300">
                      ✨ Level {currentLevel.id} Complete! {currentLevel.id < 6 ? ` Level ${currentLevel.id + 1} is now unlocked!` : ' You have completed all levels!'}
                    </p>
                  </div>
                )}
                <div className="flex justify-center space-x-4">
                  <button
                    onClick={() => {
                      ...
                      setFeedback([]);
                      setScore(0);
                      setScenario(generateLevel1Scenario());
                    }}
                    className="px-6 py-3 bg-blue-500 hover:bg-blue-600 rounded-lg font-semibold transition-colors"
                  >
                    Try Again
                  </button>
                  <button onClick={() => setCurrentView('building')} className="px-6 py-3 bg-green-500 hover:bg-green-600 rounded-lg font-semibold transition-colors">
                    Back to Building
                  </button>
                </div>
              </div>
...
const Input = ({
  label, placeholder, tabIndex, defaultValue, onBlur, className = ''
}: {
  label: string; placeholder: string; tabIndex: number; defaultValue?: string; onBlur: (v: string) => void; className?: string;
}) => (
